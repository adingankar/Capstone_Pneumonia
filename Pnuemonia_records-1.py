...
from config import pnuemonia_config as config
...
def main(_):
	...
	for row in rows[1:]:     
		...
		row = row.split(',')
		(PId, x, y,  width, height,label) = row
        
        #(PId, x, y, width, height,Target, _) = row 
		...
		p = os.path.sep.join([config.BASE_PATH, "images\\check\\{}.png".format(PId.strip(''))])
		b = D.get(p, [])
		...
		b.append((x, y, width, height, label))
		D[p] = b
	...
	for (dType, keys, outputPath) in datasets:
		...
		for k in keys:
            ...
            #encoded = pydicom.dcmread(k).pixel_array
			...
			# the image
			for (x,y,width, height,label) in D[k]:
